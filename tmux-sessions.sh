...
if [ $? != 0 ]; then
  ...
  tmux split-window -c packages/client -h "npm run watch:chrome-extension; bash -i"
  tmux split-window -c packages/client -v "npm run serve:standalone-devtools; bash -i"
