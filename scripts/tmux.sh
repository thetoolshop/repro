...
if [ $? != 0 ]; then
  tmux new-session -s $SESSIONNAME -n client -c $BASE_PATH/apps/repro/client -d
  tmux split-window -c $BASE_PATH/apps/repro/client -h "npm run watch:capture; bash -i"
  tmux split-window -c $BASE_PATH/apps/repro/client -v "npm run serve:main; bash -i"
